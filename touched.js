...
function insertBox(arg, item, keepIds) {
	...
	if(!keepIds) {
		var id = 1;
		item.find('.box,.group').andSelf().each(function(i, d) {
			while($('#box-' + id).length > 0)
			id++;
			$(d).attr('id', 'box-' + id);
		});
		...
		if(hasMoved || Math.abs(startPos[0] - evt.clientX) + Math.abs(startPos[1] - evt.clientY) > 20) {
			...
			if(!hasMoved && !blocked) {
				...
				while(!hand.hasClass('element')) {
					if(hand.is(canvas) || hand.length == 0)
						return;
					hand = hand.parent();
				}
				startOffset = {
					top : hand.offset().top - startPos[1],
...
function setCodeAttribute(id, attrname, attrvalue) {
	...
	if(obj[0]) {
		...
		for(var i = 0; i < arg.length; i++) {
			if(arg[i].getAttribute('data-name') == attrname) {
				arg[i].children[0].innerHTML = attrvalue;
				return;
			}
		}
	}
